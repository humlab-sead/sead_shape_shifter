...
class TestRunOptions(BaseModel):
    ...
    entities: Optional[List[str]] = Field(
        None, description="List of entity names to test. If None, tests all entities"
    )
    max_rows_per_entity: int = Field(
        100, description="Maximum number of rows to process per entity", ge=10, le=10000
    )
    output_format: OutputFormat = Field(
        OutputFormat.PREVIEW, description="Output format for results"
    )
...
class EntityTestResult(BaseModel):
    ...
    preview_rows: Optional[List[Dict[str, Any]]] = Field(
        None, description="Preview of output rows (first 10)"
    )
...
class TestRunResult(BaseModel):
    ...
    entities_processed: List[EntityTestResult] = Field(
        default_factory=list, description="Results for each entity"
    )
...
class TestRunRequest(BaseModel):
    ...
    options: TestRunOptions = Field(
        default_factory=TestRunOptions, description="Test run options"
    )
