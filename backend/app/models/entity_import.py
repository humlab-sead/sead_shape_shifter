...
class EntityImportRequest(BaseModel):
    ...
    entity_name: Optional[str] = Field(
        None,
        description="Optional custom entity name (defaults to table name)"
    )
    include_all_columns: bool = Field(
        True,
        description="Include all columns from the table"
    )
    selected_columns: Optional[List[str]] = Field(
        None,
        description="Specific columns to include (if not include_all_columns)"
    )
...
class EntityImportResult(BaseModel):
    ...
    surrogate_id_suggestion: Optional[KeySuggestion] = Field(
        None,
        description="Suggested surrogate ID column"
    )
    natural_key_suggestions: List[KeySuggestion] = Field(
        default_factory=list,
        description="Suggested natural key columns"
    )
    column_types: dict[str, str] = Field(
        default_factory=dict,
        description="Column names to data types"
    )
