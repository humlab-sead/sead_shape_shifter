...
class DependencySuggestion(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "entity": "users",
                "reason": "Entity has foreign key to users table",
                "confidence": 0.95
            }
        }
...
class EntitySuggestions(BaseModel):
    ...
    foreign_key_suggestions: List[ForeignKeySuggestion] = Field(
        default_factory=list,
        description="Suggested foreign key relationships"
    )
    dependency_suggestions: List[DependencySuggestion] = Field(
        default_factory=list,
        description="Suggested processing dependencies"
    )
