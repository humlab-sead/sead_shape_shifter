...
class QueryResult(BaseModel):
    ...
    total_rows: Optional[int] = Field(
        default=None,
        description="Total number of rows in result (if known, before truncation)"
    )
...
class QueryValidation(BaseModel):
    ...
    is_valid: bool = Field(
        ...,
        description="Whether the query is valid and safe to execute"
    )
    errors: List[str] = Field(
        default_factory=list,
        description="Validation errors (syntax, security, etc.)"
    )
    warnings: List[str] = Field(
        default_factory=list,
        description="Non-fatal warnings about the query"
    )
...
class QueryPlan(BaseModel):
    ...
    plan_text: str = Field(
        ...,
        description="Formatted query execution plan"
    )
