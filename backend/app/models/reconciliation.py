...
class ReconciliationRemote(BaseModel):
    ...
    columns: list[str] = Field(
        default_factory=list, description="Additional columns for display/matching"
    )
...
class ReconciliationMapping(BaseModel):
    ...
    confidence: float | None = Field(
        None, ge=0.0, le=1.0, description="Mapping confidence (0-1)"
    )
...
class EntityReconciliationSpec(BaseModel):
    ...
    source: str | ReconciliationSource | None = Field(
        None,
        description="Data source: None (entity preview), entity name (string), or custom query (ReconciliationSource)"
    )
    keys: list[str] = Field(..., description="Local key fields for matching")
    columns: list[str] = Field(
        default_factory=list, description="Additional columns for context/matching"
    )
    ...
    remote: ReconciliationRemote = Field(
        ..., description="Remote SEAD entity specification"
    )
    ...
    mapping: list[ReconciliationMapping] = Field(
        default_factory=list, description="Manual and auto-generated mappings"
    )
...
class ReconciliationConfig(BaseModel):
    ...
    service_url: str = Field(
        ..., description="Base URL of OpenRefine reconciliation service"
    )
    entities: dict[str, EntityReconciliationSpec] = Field(
        default_factory=dict, description="Entity name -> reconciliation spec"
    )
...
class ReconciliationCandidate(BaseModel):
    ...
    type: list[dict[str, str]] = Field(
        default_factory=list, description="Entity type info"
    )
    distance_km: float | None = Field(
        None, description="Geographic distance (for sites)"
    )
...
class AutoReconcileResult(BaseModel):
    ...
    candidates: dict[str, list[ReconciliationCandidate]] = Field(
        default_factory=dict, description="Source key -> candidates mapping"
    )
