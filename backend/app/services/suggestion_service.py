...
class SuggestionService:
    ...
    async def suggest_for_entity(
        ...
        return EntitySuggestions(
            ...
            foreign_key_suggestions=fk_suggestions,
            dependency_suggestions=dep_suggestions
        )
    ...
    async def suggest_foreign_keys(
        ...
        entity: Dict[str, Any],
        all_entities: List[Dict[str, Any]],
        schemas: Dict[str, TableSchema] = None
        ...
        for local_col in local_columns:
            ...
            if local_col in remote_columns:
                matches.append({
                    "local": local_col,
                    "remote": local_col,
                    "reason": f"Exact column name match: '{local_col}'",
                    "match_type": "exact"
                })
                continue
    ...
    def _infer_dependencies_from_foreign_keys(
        self,
        fk_suggestions: List[ForeignKeySuggestion]
        ...
        for fk in fk_suggestions:
            ...
            if fk.confidence >= 0.7 and fk.remote_entity not in seen_entities:
                dependencies.append(DependencySuggestion(
