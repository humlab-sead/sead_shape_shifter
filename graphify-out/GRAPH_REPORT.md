# Graph Report - sead_shape_shifter  (2026-09-08)

## Corpus Check
- 857 files · ~799,536 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 18973 nodes · 42282 edges · 765 communities (650 shown, 115 thin omitted)
- Extraction: 75% EXTRACTED · 25% INFERRED · 0% AMBIGUOUS · INFERRED: 10542 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9a0b65f8`
...
2. `ShapeShiftProject` - 556 edges
3. `Project` - 425 edges
4. `ProjectService` - 376 edges
5. `TableStore` - 348 edges
6. `Action` - 292 edges
7. `AuthorizedResource` - 258 edges
8. `ProjectMapper` - 215 edges
...
## Communities (765 total, 115 thin omitted)

### Community 0 - "Project"
Cohesion: 0.05
Nodes (57): CoreIssue, ValidationError, ValidationIssue, ConformanceIssue, Mapper for translating between domain validation models and API validation model, Mapper for validation layer boundaries (Domain ↔ API)., Convert any CoreIssue subclass to an API ValidationError.          Args:, Convert a domain ValidationIssue to an API ValidationError. (+49 more)

### Community 1 - "ProjectService"
Cohesion: 0.01
Nodes (112): Path, ProjectService, Test load-modify-save-reload cycle., Test get_project_service returns singleton instance., Create temporary configurations directory., Create service instance with temporary directory., Create a sample project with additional files., Test copying project and all its files. (+104 more)

### Community 2 - "derive_fixed_schema"
Cohesion: 0.03
Nodes (42): Calculate completion statistics., Mark entity as complete.          Validates that entity passes validation and ha, Refresh task state from sidecar without touching cached project state., Mark entity as ignored.          Args:             project_name: Name of the pro, Reset entity status to todo.          Args:             project_name: Name of th, Mark entity as ongoing.          Args:             project_name: Name of the pro, Mark entity as todo (planned but not yet created).          Args:             pr, Toggle flagged status for an entity.          Args:             project_name: Na (+34 more)

### Community 3 - "specifications/test_entity.py"
Cohesion: 0.04
Nodes (43): Path, Any, Path, Settings, Path, BaseSettings, Require trusted-proxy authentication for production settings., Resolve relative paths against APPLICATION_ROOT and ensure directories exist. (+35 more)

### Community 4 - "TableConfig"
Cohesion: 0.01
Nodes (191): GlobalConstraint, Tests for append configuration parsing and validation., Test parsing of append configurations from YAML-like dictionaries., Test dependency resolution for append configurations., Test that append source is added to depends_on., Test multiple append sources are added to depends_on., Test entity without append configuration., Test that fixed append doesn't add dependencies. (+183 more)

### Community 5 - "AutoFixService"
Cohesion: 0.02
Nodes (143): Reconciliation service module.  This module provides services for entity reconci, EntityResolutionSet, ProjectService, ShapeShiftProject, Any, AutoReconcileResult, EntityResolutionCatalog, EntityResolutionSet (+135 more)

### Community 6 - "FixedLoader"
Cohesion: 0.08
Nodes (44): Call, ColumnRef, DSLException, DSLValidationError, Expr, extract_column_references(), FunctionSpec, Literal (+36 more)

### Community 7 - "SimsClient"
Cohesion: 0.03
Nodes (71): Any, AsyncClient, ReconciliationCandidate, HTTP client for OpenRefine reconciliation service API., Check if reconciliation service is available.          Returns:             dict, Get entity suggestions for autocomplete.          Args:             prefix: Text, Get HTML preview for entity.          Args:             entity_id: Entity URI, Get reconciliation service metadata.          Returns:             Service manif (+63 more)
...
Cohesion: 0.08
Nodes (16): Tests for CSV file loader., Should handle invalid CSV format gracefully., Tests for CSV loader., Should handle empty CSV file., Should successfully test connection to existing CSV file., Should fail when CSV file doesn't exist., Should fail when filename is not provided., Should test connection with custom CSV separator. (+8 more)

### Community 11 - "FixedEntityPersistenceStrategy"
Cohesion: 0.04
Nodes (111): HttpGroupMembershipResolver, MembershipLookupStatus, Trusted group-membership lookup for authorization review., Result states returned by a membership provider., Read group members from a configured trusted HTTP provider., Fetch one group's members without changing authorization state., _apply_administrators(), _apply_grants() (+103 more)

### Community 12 - "TaskService"
Cohesion: 0.05
Nodes (53): Index, DataFrame, Series, PolicyBase, AddIdentityMappingSystemIdToPublicIdPolicy, AddPrimaryKeyColumnIfMissingPolicy, DisabledError, DropIgnoredColumns (+45 more)

### Community 13 - "ProjectDetailView.vue"
Cohesion: 0.02
Nodes (129): DataSourceConfig, Any, TableSchema, AuthorizationService, DataSourceConfig, DataSourceStatus, DataSourceTestResult, Principal (+121 more)

### Community 14 - "UCanAccessSqlLoader"
Cohesion: 0.04
Nodes (40): DataSourceConfig, QueryService, Return a DataSourceConfig for use in execute_query calls., Should reject destructive queries with structured error., Test QueryService for SQL query validation and execution., Create QueryService instance., Return a DataSourceConfig for use in execute_query/introspect calls., Test validation of valid SELECT query. (+32 more)

### Community 15 - "schema_reference.py"
Cohesion: 0.07
Nodes (75): Any, AuthorizedResource, AutoReconcileResult, Depends, EDIT, EntityResolutionCatalog, EntityResolutionListItem, READ (+67 more)
...
Cohesion: 0.03
Nodes (60): DispatcherRegistry, Registry for dispatcher classes., Environment, Any, Connection, Any, DataFrame, Self (+52 more)

### Community 21 - "ValidationError"
Cohesion: 0.06
Nodes (62): Any, DataFrame, ProjectService, ShapeShiftProject, ShapeShiftService, TableStore, TargetModel, ValidationIssue (+54 more)
...
Cohesion: 0.05
Nodes (53): DirectiveResolver, EnvironmentVariableResolver, find_unresolved_directives(), IncludeResolver, _is_path_env_var(), load_resolved_yaml(), LoadResolver, _raise_on_unresolved_directives() (+45 more)

### Community 24 - "get_data_source_status"
Cohesion: 0.02
Nodes (203): Any, AuthorizedResource, DataValidationMode, Depends, EDIT, READ, require_project, Any (+195 more)

### Community 25 - "EntityValuesService"
Cohesion: 0.07
Nodes (52): Any, IngesterConfig, IngestionResult, Build a standard failed ingestion result., Configuration for an ingester instance.      This is the base configuration clas, Initialize ingester with configuration.          Args:             config: Inges, Result of ingestion operation.      Attributes:         success: Whether ingesti, Build a standard successful ingestion result. (+44 more)

### Community 26 - "UnnestConfig"
Cohesion: 0.04
Nodes (34): Unit tests for arbodat utility configuration classes., Test creating a valid unnest configuration., Test that missing unnest key raises ValueError., Test that missing id_vars is allowed (defaults to empty list)., Test that missing value_vars is allowed (defaults to empty list)., Test that missing var_name raises ValueError., Test that missing value_name raises ValueError., Test that empty lists are allowed. (+26 more)

### Community 27 - "src/utility.py"
Cohesion: 0.06
Nodes (75): SubmissionContext, TargetModel, ChangeRequestTable, datetime, DeployArtifact, ChangeRequestPackage, IdentityResolutionResult, PlannedTable (+67 more)

### Community 28 - "CreateEntityFromTableDialog.vue"
Cohesion: 0.08
Nodes (39): _minimal_project(), Classifier source-type mismatches should stay advisory unless overridden., Project severity overrides should replace the default severity for a conformance, The registry should execute both global and entity validator subclasses., PublicIdConformanceValidator produces no issue when the target spec does not dec, Disabled conformance rules should be skipped by registry key., If non-required entity X is in the project and has a required FK to Y,     then, No issue when the induced-required entity Y is already present in the project. (+31 more)

### Community 29 - "IdentityResolutionResult"
Cohesion: 0.04
Nodes (93): TargetModel, TargetModel, DataFrame, EntitySpec, IdentityResolutionResult, TargetModel, TargetProjectionResult, IdentityResolutionResult (+85 more)

### Community 30 - "app/models/__init__.py"
Cohesion: 0.04
Nodes (74): ApplicationRoleAssignment, AuditEvent, Centralized authorization primitives and policy services., MembershipSnapshot, A point-in-time group membership lookup result., ApplicationRole, ApplicationRoleAssignment, AuditEvent (+66 more)
...
Cohesion: 0.02
Nodes (126): CaptureFixture, DataLoader, JClass, ConnectTestResult, DataLoader, DataLoaderRegistry, LoaderType, Get loader keys filtered by loader type. (+118 more)
...
Cohesion: 0.04
Nodes (98): build_table_config(), DSL formulas should pull required source columns before evaluating extra_columns, Escaped leading '=' literals should remain constants during extraction., Test that columns with missing dependencies are silently deferred (logged but no, Test that all columns are evaluated when dependencies are present., Return a fresh TableConfig for the test entity with optional overrides., Test transform rule that applies normalize operations to all values (no match fi, Test transform rule that coerces all values (no match filter). (+90 more)

### Community 36 - ".is_interpolated_string"
Cohesion: 0.05
Nodes (27): Structural/configuration issue emitted by specification checks.      Additional, Add an error message., Add a warning message., SpecificationIssue, Tests for SpecificationIssue class., Test creating an error issue., Test creating a warning with field context., Test string representation with minimal info. (+19 more)
...
Cohesion: 0.01
Nodes (139): Return cleaned database options., Data source configuration mapper between API and core models., Unit tests for arbodat utility configuration classes., Tests for helper functions in src.utility., Should strip accents and lowercase to mimic Postgres unaccent behavior., dotset creates nested dicts; dotget resolves colon/underscore variants., Environment variables with prefix populate nested dict with colon splitting., Environment placeholders get replaced in nested structures. (+131 more)

### Community 42 - "AddIdentityMappingSystemIdToPublicIdPolicy"
Cohesion: 0.09
Nodes (43): Create a dependency that authorizes an application-scoped action., Authorize access to an operation owned by the current principal and project., Create a dependency that authorizes a shared data source locator for an action., require_application_action(), require_operation(), require_shared_data_source(), Server-owned resource record and current locator., Return the stable identity without exposing its locator. (+35 more)
...
Cohesion: 0.05
Nodes (43): Column, SeadSchema, Submission, SeadSchema, Column, ColumnTypesSpecification, ForeignKeyColumnsHasValuesSpecification, ForeignKeyExistsAsPrimaryKeySpecification (+35 more)
...
Cohesion: 0.14
Nodes (29): issue_pairs(), load_project(), load_real_project(), load_target_model(), _minimal_target_model(), PublicIdConformanceValidator reports MISSING_PUBLIC_ID when the spec declares a, If X (present) → Y (absent, required FK) → Z (absent, required FK), both Y and Z, A classifier using type: fixed is correct — no issue. (+21 more)
...
Cohesion: 0.03
Nodes (72): expected_bundle_name(), FakeCollisionChecker, FakeReconciliationClient, FakeSimsClient, minimal_submission_context(), minimal_target_model(), Tests for the SEAD change request ingester scaffold behavior., Ingest should emit the Delivery 1 artifact bundle when assignments are present. (+64 more)
...
### Community 58 - "Any"
Cohesion: 0.06
Nodes (24): DataFrame, _demo(), FormulaEngine, Extra columns evaluation with support for constants, column copies, interpolated, Evaluate simple formula., Evaluate complex nested formula., Apply multiple extra_columns formulas to DataFrame., Apply extra_columns in place modifies original DataFrame. (+16 more)
...
### Community 60 - "Table"
Cohesion: 0.10
Nodes (35): Any, EntitySuggestions, SchemaIntrospectionService, TableSchema, DependencySuggestion, ForeignKeySuggestion, Metadata about a database table., TableMetadata (+27 more)

### Community 61 - "replace_env_vars"
Cohesion: 0.15
Nodes (27): AuthorizedResource, Depends, EDIT, require_project, _build_entity_response(), create_entity(), delete_entity(), EntityResponse (+19 more)

### Community 62 - "IngesterConfig"
Cohesion: 0.04
Nodes (112): TargetModel, SubmissionContext, IdentityOrchestrationResult, IdentityAssignment, IdentityResolutionResult, PlannedTable, ResolvedIdentityTable, TargetModel (+104 more)
...
Cohesion: 0.05
Nodes (41): SubmissionContext, Any, DataFrame, EntitySpec, PlannedTable, SourceTableBundle, FakeReconciliationClient, FakeSimsClient (+33 more)
...
Cohesion: 0.05
Nodes (60): ReconciliationCandidate, Any, Identifier, ReconciliationClient, Settings, SubmissionContext, ReconciliationCandidate, SubmissionContext (+52 more)
...
### Community 71 - "Any"
Cohesion: 0.01
Nodes (231): Any, AuthorizedResource, Depends, EDIT, ProjectTaskStatus, READ, require_project, YamlService (+223 more)
...
Cohesion: 0.02
Nodes (157): AuthorizationService, AuthorizedResource, DataSourceConfig, DataSourceService, DataSourceStatus, DataSourceTestResult, Depends, get_authorization_service (+149 more)
...
### Community 77 - "unnest"
Cohesion: 0.09
Nodes (14): Test IngestionResult dataclass., Tests for the SeadIngester class., Test getting SeadIngester metadata., Test creating a SeadIngester instance., Test ValidationResult dataclass., TestSeadIngester, Ingest Excel file into SEAD database.          This performs the complete ingest, Ingester for SEAD Clearinghouse database.      This ingester handles the complet (+6 more)
...
Cohesion: 0.12
Nodes (20): MonkeyPatch, TableSchema, ColumnMetadata, build_schema(), Tests for SuggestionService foreign key and dependency logic., _get_table_schemas should load schemas for matching entities, respecting case-in, Helper to build a TableSchema., If table listing fails, no schemas are loaded. (+12 more)
...
Cohesion: 0.03
Nodes (103): CsvProcessor, Main class that processes the Submission and produces CSV files directly.      C, IDispatcher, Column, SeadSchema, Table, SeadSchema, SeadSchemaFactory (+95 more)
...
Cohesion: 0.06
Nodes (31): Identifier, SQL Query execution service for the Shape Shifter Configuration Editor., extract_select_columns(), extract_tables(), get_statement_type(), has_wildcard_select(), Extract table names from an SQL query using sqlparse.      Returns a sorted, ded, Return the SQL statement type (SELECT, INSERT, etc.) from a parsed statement. (+23 more)
...
Cohesion: 0.01
Nodes (135): Path, Project, ProjectMetadata, Path, Settings, Any, Path, Project (+127 more)
...
Cohesion: 0.03
Nodes (54): Interpolation dependencies should be extracted from source even if not selected, DSL dependencies should be extracted from source even if not selected explicitly, test_collect_source_dependencies_includes_dsl_refs(), test_collect_source_dependencies_includes_interpolation_refs(), Initialize SubsetService with ExtraColumnEvaluator., ExtraColumnEvaluator, Evaluates extra_columns with support for constants, copies, interpolated strings, Convert escaped braces to literal braces.          Args:             text: Strin (+46 more)

### Community 100 - "FormulaEngine"
Cohesion: 0.05
Nodes (28): DataFrame, Test query execution functionality., Set up test fixtures., Mock the loader's read_sql method., Should execute SELECT query and return results., Should apply LIMIT clause to query., Should detect truncated results., Should reject stacked statements before constructing or calling a loader. (+20 more)

### Community 101 - "EntitySpec"
Cohesion: 0.15
Nodes (25): ForeignKeyConstraintValidator, ForeignKeyRuntimeOptions, ForeignKeyConstraintValidator, ForeignKeyRuntimeOptions, Orchestrates validation of foreign key constraints during and after merging., Return opts required for merge validation., Resolved runtime behavior for foreign-key null handling., Lookup-style runtime options should bypass strict null-key validation. (+17 more)
...
Cohesion: 0.08
Nodes (20): DuckDbLoader, SQL loader over Shape Shifter's internal table_store.      This loader does not, Load an internal DuckDB-derived entity., DuckDbWorkspace, Copy a DataFrame into DuckDB as a physical table.          Usually not needed in, Persistent DuckDB workspace over Shape Shifter's TableStore.      DataFrames are, Queue an entity DataFrame for registration on the next query., Register or re-register an entity DataFrame immediately in DuckDB. (+12 more)
...
Cohesion: 0.04
Nodes (24): Unit tests for DeferredLinkingTracker., Test that DeferredLinkingTracker initializes with empty deferred set., Test that track method uses named parameters correctly., Test that deferred set can be directly manipulated (as it's a public attribute)., Test tracking with empty entity name (edge case)., Test tracking entities with special characters in names., Test that independent DeferredLinkingTracker instances don't share state., Test tracking a single entity as deferred. (+16 more)
...
Cohesion: 0.08
Nodes (23): lifespan(), Application lifespan events., Path, Create an authenticated client backed by an isolated authorization store.      T, get_settings(), Application configuration., Get cached settings instance., configure_logging() (+15 more)
...
Cohesion: 0.17
Nodes (13): load_example_as_core_project(), load_example_project(), load_target_model(), The canonical minimal example should also run through the validator cleanly., The fixture that deliberately omits sample_group should produce a MISSING_REQUIR, All example YAMLs should run through the conformance engine without crashing., Use TargetModelConformanceValidator to check example projects against sead_stand, The complete arbodat example should produce no critical conformance errors. (+5 more)

### Community 119 - "ShapeShifter"
Cohesion: 0.03
Nodes (62): ABC, ForeignKeyConstraints, Tests for ForeignKeyConstraints class., Test constraints with no data., Test constraints with empty dict., Test cardinality property., Test allow_unmatched_left property., Test allow_unmatched_right property. (+54 more)
...
Cohesion: 0.02
Nodes (271): Create a dependency that authorizes a project locator for an action., require_project(), Action, AuthorizedResource, Operations that can be authorized., Resource returned after a successful authorization decision., Any, MaterializationService (+263 more)
...
Cohesion: 0.08
Nodes (17): _df(), Unit tests for merge_with_null_safety and related helpers in src/transforms/util, allow_null_keys=True should activate null-safe mode., Without nulls in keys, null-safe mode uses pd.merge directly., YAML strings like '1'/'17' joining DB int column., Numeric left, string right — coerces right side., Object key with non-numeric values (real strings) must not be coerced., Coercion must not inflate NaN counts (safety guard). (+9 more)

### Community 137 - "Full Manual Checklist"
Cohesion: 0.04
Nodes (41): ApplicationState, Path, Project, app_state(), project_dir(), Test ApplicationState class., Test ApplicationState initialization., Test starting and stopping background tasks. (+33 more)
...
Cohesion: 0.04
Nodes (49): default, description, title, type, anyOf, default, description, title (+41 more)
...
Cohesion: 0.02
Nodes (184): Any, Settings, Any, Entity, Project, ShapeShiftProject, Path, Settings (+176 more)

### Community 146 - "Full Manual Checklist"
Cohesion: 0.12
Nodes (13): Path, ProjectFileInfo, UploadFile, Sanitize uploaded filename to prevent path traversal.          Args:, List files stored under a project's uploads directory.          Args:, Save an uploaded file into the project's uploads directory.          Args:, List files available for data source configuration.          Args:             e, Initialize file manager.          Args:             projects_root: Base director (+5 more)
...
Cohesion: 0.07
Nodes (24): DataFrame, DataSourceConfig, SchemaIntrospectionService, Settings, ColumnMetadata, Metadata about a table column., Test column metadata., Test column metadata with default values. (+16 more)
...
Cohesion: 0.05
Nodes (35): EndsWithIdValidator, FieldIsNotEmptyStringValidator, IsEmptyFieldValidator, IsExistingEntityValidator, Validator to check that a field is an existing entity., Validator to check that a field ends with '_id'., Validator to check if a field is empty (None, empty string, empty list, or empty, Validator to check if a field is a non-empty string. (+27 more)
...
Cohesion: 0.09
Nodes (21): Acceptance Criteria, Alternatives Considered, API contract, Constrained relative paths only, Current Behavior, Final Recommendation, Keep operation state in memory, Lifecycle and consistency (+13 more)

### Community 159 - "QueryBuilder.vue"
Cohesion: 0.05
Nodes (17): Any, Get ingester configurations from project options., Get configuration for a specific ingester., Initialize ForeignKeyConfig with configuration data.         Args:             l, Get extra columns {"new-column-name" -> value } mapping from configuration., Resolve extra columns for the foreign key configuration.          The mapping re, Get list of valid remote columns to select from the remote dataframe based on th, Generate the setup for linking based on the foreign key configuration. (+9 more)
...
### Community 161 - "Any"
Cohesion: 0.04
Nodes (48): Integration tests for append feature end-to-end functionality., config_with_append(), config_with_distinct_mode(), config_with_source_append(), Integration tests for append processing in the normalization pipeline., Tests for SQL-based append processing., Test appending data from SQL query., Create sample survey data for testing. (+40 more)
...
Cohesion: 0.14
Nodes (12): Any, Path, Load a specific data source by filename.          If it's a dict, assume it's al, Create a new global data source file.          Args:             filename: Filen, Update an existing data source file.          Args:             filename: Curren, Delete a data source file.          Args:             filename: Data source file, Get current status of a data source.          Args:             filename: Data s, Resolve the full path to a data source file. (+4 more)
...
Cohesion: 0.10
Nodes (13): ValidationService, Create ValidationService instance., Tests for basic validation functionality., Test validating empty configuration., Test data validation groups issues by severity using dependency injection., Test validating valid configuration., Projects without a target model should skip conformance cleanly., A resolved null target_model should behave the same as an absent target model. (+5 more)
...
Cohesion: 0.01
Nodes (195): Any, create_valid_config_dict(), Tests for ProjectMapper., Test entity with foreign keys., Tests for to_core_dict method., Helper to create a valid config dict with all required sections., Tests for _dict_to_api_entity method., Test minimal entity conversion. (+187 more)
...
Cohesion: 0.02
Nodes (187): AuthorizedResource, Depends, EDIT, EntityMapping, Link, READ, require_project, EntityMapping (+179 more)
...
Cohesion: 0.15
Nodes (18): build_fk(), build_project(), Tests for constraint validators and orchestration., Rows with an existing public_id are treated as simple mappings and are exempt fr, Strict null-key validation still applies to rows whose public_id is not populate, Remote rows with an existing public_id are exempt from strict null-key validatio, Remote rows without a public_id still need non-null join keys when strict valida, Registry returns stage-filtered and sub-key lookups. (+10 more)

### Community 191 - "Playwright E2E Tests"
Cohesion: 0.06
Nodes (48): EntityConflictError, Entity was concurrently modified; ETag mismatch.      Raised when a PUT /entitie, Any, Entity, Project, Any, EntityOperations, Project (+40 more)
...
Cohesion: 0.25
Nodes (13): FileLocation, Path, Resolve a managed file path against the configured global or local root., resolve_managed_file_path(), Path, test_resolve_managed_file_path_allows_missing_parent_directories(), test_resolve_managed_file_path_global_relative(), test_resolve_managed_file_path_local_relative() (+5 more)
...
Cohesion: 0.12
Nodes (18): DSLParseError, Parser, A token with position information., Hand-written recursive descent parser., Parse a formula starting with '='., Parse an expression: function_call | column_ref | literal., Parse a function call: NAME "(" [arg_list] ")"., Parse a column reference: NAME. (+10 more)

### Community 207 - "test_example_projects.py"
Cohesion: 0.05
Nodes (42): anyOf, default, title, title, type, items, title, type (+34 more)
...
Cohesion: 0.06
Nodes (26): get_schema_service(), Get SchemaIntrospectionService instance.      Creates service with current confi, client(), mock_schema_service(), Tests for schema introspection API endpoints., Test table listing with specific schema., Test with database error., Tests for GET /data-sources/{name}/tables/{table_name}/schema endpoint. (+18 more)

### Community 226 - "type"
Cohesion: 0.21
Nodes (17): EntitySuggestions, SchemaIntrospectionService, analyze_entities(), get_suggestion_service_dep(), API endpoints for entity relationship suggestions., Dependency injection for suggestion service., Analyze a set of entities and generate relationship suggestions.      **Request, Get relationship suggestions for a single entity.      **Request Body**:     - ` (+9 more)
...
Cohesion: 0.04
Nodes (49): global_exception_handler(), FastAPI application entry point., Serve SPA for all routes (except API and static assets)., Root endpoint - redirect to docs (API-only mode)., Catch all unhandled exceptions and log with full traceback.      This is a last-, root(), serve_spa(), Verify every declared protected endpoint rejects a request before handler valida (+41 more)
...
Cohesion: 0.06
Nodes (34): items, title, type, additionalProperties, properties, title, type, ColumnSpec (+26 more)
...
Cohesion: 0.07
Nodes (35): Alerting, Approved file roots, Authorization ownership and recovery, Authorization SQLite store, Automatic project backups, Backup and recovery, Backup and Recovery, Build and deploy (+27 more)

### Community 237 - "Proposal: BugsCEP Importer Migration Runtime Decision Spike"
Cohesion: 0.03
Nodes (62): ProjectSpecification, Get the TableConfig for the specified entity., Check if a specific entity exists in the configuration., Check if a specific field exists in the entity configuration., Base specification for project validation., EntitySpecification, Validates that entities are properly configured.      Composite specification th, Get the list of specifications to run for entity validation.          Override t (+54 more)

### Community 238 - "Playwright E2E Testing Setup - Complete! ✅"
Cohesion: 0.04
Nodes (48): Any, PreviewResult, ProjectService, ShapeShiftProject, TableConfig, TableStore, ShapeShiftProject, TableConfig (+40 more)
...
Cohesion: 0.12
Nodes (16): 1. Define And Centralize The SQL Safety Policy, 2. Secure Query Construction And Result Limits, 3. Enforce PostgreSQL Least-Privilege Execution, 4. Restrict DuckDB External Access And Extensions, 5. Add Query Resource Controls And Regression Evidence, Assumptions, Definition Of Done, Deliverables (+8 more)
...
Cohesion: 0.18
Nodes (9): Any, Get list of tables in a data source.          Args:             data_source: Eit, Get detailed schema for a specific table.          Args:             data_source, Get preview of table data.          Args:             data_source_cfg: Either a, Generate entity configuration from a database table.          Args:, Convert value to string or None, handling pandas NaN.      Args:         value:, Get cached value if not expired., Check if a valid cache entry exists. (+1 more)

### Community 244 - "resolve.py"
Cohesion: 0.13
Nodes (14): API Routes, Authorization Route Inventory, Entities, Directives, And Validation, Help And Sessions, Ingester, Filters, Logs, And Release Notes, Maintenance, Mapping, Execution, And Materialization, Projects (+6 more)
...
Cohesion: 0.11
Nodes (17): Tests for DataSourceMapper., Test mapping PostgreSQL configuration., Test error when required field is missing., Test that additional options not in schema are preserved., Test PostgreSQL with default values., Test mapping MS Access configuration., Mapper should use canonical file path from options., Test mapping SQLite configuration. (+9 more)
...
Cohesion: 0.16
Nodes (16): AsyncClient, SQLiteAuthorizationRepository, authorization_repository(), _client_for_principal(), mock_data_source_service(), query_dependencies(), Tests for query endpoint authorization responses., Create a client that optionally supplies a trusted-proxy identity. (+8 more)
...
### Community 262 - "error"
Cohesion: 0.15
Nodes (14): FakeFK, FakeTableConfig, test_get_subset2_column_aliases_copy_source_columns_under_requested_names(), test_get_subset2_drop_duplicates_list_drops_duplicates_on_subset(), test_get_subset2_drop_duplicates_true_drops_exact_duplicates(), test_get_subset2_drop_empty_list_only_considers_listed_columns(), test_get_subset2_drop_empty_true_drops_rows_all_empty_after_subsetting(), test_get_subset2_extracts_columns_and_adds_extra_source_and_constant_columns() (+6 more)

### Community 263 - "ForeignKeyEditor.vue"
Cohesion: 0.14
Nodes (20): QueryExecutionError, QuerySecurityError, Query execution failed.      Common causes:     - Data source connection issues, Query contains prohibited operations.      Occurs when query attempts destructiv, DataSourceConfig, QueryResult, QueryValidation, Statement (+12 more)
...
Cohesion: 0.03
Nodes (115): get_authenticated_user(), get_current_session(), get_data_source_service(), get_session_id(), API Dependencies  Provides dependency injection functions for FastAPI endpoints., Get DataSourceService instance.      Creates service for managing global data so, Extract session ID from header or cookie., Return the identity asserted by the trusted proxy, when proxy authentication is (+107 more)

### Community 270 - "MaterializationConfig"
Cohesion: 0.14
Nodes (13): Centralized Authorization System Cutover Plan, Cross-Phase Rules, Current Position, Final Recommendation, Phase 1: Complete Route And Operation Inventory, Phase 2: Review Deployment Resources And Initial Grants, Phase 3: Validate Migration And Cutover Readiness, Phase 4: Execute And Record Enforcement Cutover (+5 more)
...
Cohesion: 0.07
Nodes (22): ColumnType, FieldValidator, Get the configuration for a specific entity., Check fields based on the specified check type.          Args:             entit, Get specified types of result columns available.         FIXME: consider moving, Validate a specific field in the entity configuration, or target_cfg if provided, Apply the validation rule to the specified field., Log the failure of the validation rule. (+14 more)
...
Cohesion: 0.07
Nodes (25): Dtype, DataFrame, ConfigLike, SchemaService, SeadSchema, Submission, Full schema loaded from CSV - session scoped, use for integration tests only., ItemsView (+17 more)
...
Cohesion: 0.12
Nodes (15): Apply the server result cap while preserving a lower user limit., _find_forbidden_operation(), _has_executable_tokens(), Statement, Result of validating SQL against the shared read-only execution policy., Return whether the query may be executed., Return whether a parsed statement contains executable tokens., Return the first SQL operation token in a statement. (+7 more)
...
Cohesion: 0.15
Nodes (10): EntityReferencesExistSpecification, Validates that all referenced entities exist in the configuration.     Note: The, Tests for EntityReferencesExistSpecification., Sample project configuration., Test validation passes when all references exist., Test validation fails for missing source reference., Test validation fails for missing dependency reference., Test validation fails for missing foreign key reference. (+2 more)

### Community 310 - "models/mapping.py"
Cohesion: 0.04
Nodes (97): AuthorizedResource, Depends, EDIT, MaterializationResult, MaterializationService, MaterializedMappingSyncResult, READ, require_project (+89 more)
...
Cohesion: 0.20
Nodes (13): AsyncClient, authorization_repository(), _client_for_principal(), operation_dependencies(), Tests for operation endpoint authorization responses., Create an isolated store with the project that owns an operation., Create an operation and retain the real authorization dependency., Create a client that optionally supplies a trusted-proxy identity. (+5 more)
...
Cohesion: 0.18
Nodes (7): NonFixedIdentityColumnsSpecification, Validates that non-fixed entities do not declare derived system_id columns in `c, Tests for entity-level specifications., Tests for extra_columns conflict validation., Tests for NonFixedIdentityColumnsSpecification., TestExtraColumnsConflictsSpecification, TestNonFixedIdentityColumnsSpecification
...
Cohesion: 0.32
Nodes (13): _call_middleware(), Tests for reverse-proxy identity enforcement., Run the middleware with a minimal ASGI request and collect response messages., Return the status code from collected ASGI response messages., Return the response body from collected ASGI response messages., _response_body(), _response_status(), test_proxy_auth_accepts_groups_only_when_group_source_is_enabled() (+5 more)
...
Cohesion: 0.18
Nodes (10): Application Roles, Audit Records, Authorization, Authorization Decisions, Current Coverage, Denial Behavior, Principals, Purpose (+2 more)
...
Cohesion: 0.21
Nodes (12): AsyncClient, SQLiteAuthorizationRepository, authorization_repository(), _client_for_principal(), log_dependencies(), Tests for log endpoint authorization responses., Create an isolated authorization store with one administrator., Provide temporary log files while retaining real authorization checks. (+4 more)
...
Cohesion: 0.33
Nodes (6): NamingConventionConformanceValidator, Validate that project entity public_id values conform to the target model naming, Unit tests for NamingConventionConformanceValidator., NamingConventionConformanceValidator is registered and runs via TargetModelConfo, TestNamingConventionConformance, ShapeShiftProject
...
Cohesion: 0.24
Nodes (7): DataFrame, EntitySpec, ForeignKeySpec, Check that produced values stay within the declared allowed_values set., Check that FK values in df exist in target_df's public_id column.          Args:, Check that required, non-nullable columns contain no null values.          A col, Check that column dtypes are compatible with declared logical types.          Su
...
Cohesion: 0.10
Nodes (18): Any, DataFrame, Any, EntityPersistenceStrategy, Reject malformed fixed entities before they reach YAML persistence., Resolve a strategy directly from an entity type., Strategy interface for preparing entities for persistence., Validate and normalize entity data before persistence. (+10 more)

### Community 381 - ".flush"
Cohesion: 0.07
Nodes (17): ExcelFile, Any, SchemaService, SeadSchema, Submission, DataFrame, SeadSchema, TableStore (+9 more)
...
Cohesion: 0.07
Nodes (27): $defs, EntitySpec, ForeignKeySpec, NamingConventions, title, type, additionalProperties, title (+19 more)
...
Cohesion: 0.15
Nodes (12): [1.13.0](https://github.com/humlab-sead/sead_shape_shifter/compare/v1.12.0...v1.13.0) (2026-01-20), [1.14.0](https://github.com/humlab-sead/sead_shape_shifter/compare/v1.13.0...v1.14.0) (2026-01-21), [1.19.0](https://github.com/humlab-sead/sead_shape_shifter/compare/v1.18.0...v1.19.0) (2026-01-31), [1.5.0](https://github.com/humlab-sead/sead_shape_shifter/compare/v1.4.0...v1.5.0) (2026-01-14), [1.6.0](https://github.com/humlab-sead/sead_shape_shifter/compare/v1.5.0...v1.6.0) (2026-01-14), [1.9.0](https://github.com/humlab-sead/sead_shape_shifter/compare/v1.8.0...v1.9.0) (2026-01-16), Features, Features (+4 more)
...
Cohesion: 0.14
Nodes (40): Schema introspection failed.      Common causes:     - Data source not connected, SchemaIntrospectionError, Any, AuthorizedResource, Depends, Principal, SchemaIntrospectionService, TableMetadata (+32 more)
...
Cohesion: 0.17
Nodes (18): MonkeyPatch, Path, ExecuteService, execute_service(), Tests for execution output path confinement., Create an execution service with an isolated project root., Resolve a nested file target below the project's output directory., Reject targets that leave the authorized project's output directory. (+10 more)
...
Cohesion: 0.20
Nodes (6): Test idempotent evaluation (skipping already-evaluated columns)., Evaluator skips columns that already exist in DataFrame., Calling evaluate_extra_columns multiple times is idempotent., After partial evaluation, can complete with same extra_columns dict., Skipped columns are counted and logged., TestIdempotentEvaluation
...
Cohesion: 0.05
Nodes (47): test_from_core_issue_maps_conformance_defaults(), test_from_core_issue_maps_validation_issue(), test_from_core_issue_uses_column_as_field_fallback(), format_validation_message_with_context(), Any, Shared helpers for formatting human-readable validation messages., Prefix a message with entity and field context and append expression details whe, Any (+39 more)
...
Cohesion: 0.25
Nodes (5): Integration tests for dispatcher workflow., Test writing and reading CSV data maintains integrity., Test getting and using dispatchers from registry., Test dispatching multiple related tables to CSV., TestIntegration
...
Cohesion: 0.25
Nodes (5): Test for debugging the @include directive bug in ShapeShiftService.  This test r, Tests for debugging configuration resolution issues., Test to detect unresolved @include directives in configuration.          This ca, Test that a properly resolved config doesn't have string data sources., TestConfigurationResolution
...
Cohesion: 0.07
Nodes (20): Tests for task management API endpoints., Test that entity status has all required fields., Test getting task status for non-existent project., Reset service singletons between tests., Tests for DELETE /projects/{name}/tasks/{entity} endpoint., Test successfully resetting entity status., Test that reset removes entity from ignored list., Test resetting status in non-existent project. (+12 more)
...
Cohesion: 0.08
Nodes (15): Tests for NonEmptyResultValidator., Test passes with data., Test reports warning for empty DataFrame., Tests for ForeignKeyDataValidator., Test passes when FK columns exist., Test reports errors for missing FK columns., Test no local_keys doesn't cause errors., Tests for DataTypeCompatibilityValidator. (+7 more)
...
Cohesion: 0.03
Nodes (45): client(), Validate route should preserve change-request payload fields when calling the se, Test ingestion with non-existent ingester., Test ingestion with invalid request body., Test ingestion response structure., Ingest route should preserve change-request payload fields when calling the serv, Create a fresh TestClient so app startup runs inside the test., Test GET /api/v1/ingesters endpoint. (+37 more)
...
Cohesion: 0.25
Nodes (5): Specific tests for debugging /data-sources/sead/tables endpoint., Create test client without mocked dependencies for real testing., Test with real service - use this for debugging.          To debug:         1. S, Step-by-step debugging test.          Run with: PYTHONPATH=.:backend uv run pyte, TestDebugSeadTables
...
Cohesion: 0.10
Nodes (18): 11. Advanced Features, 1. Introduction, 9. Execute and Export, After Execution, Backups, Before You Start Editing, Common Output Types, Dependencies Graph (+10 more)
...
Cohesion: 0.07
Nodes (18): Invalidate all cached data for a data source., Simple in-memory cache for schema data., Cache a value with current timestamp., Remove a cached value., Invalidate all cached data for a data source., SchemaCache, Tests for Schema Introspection Service  Tests schema discovery for PostgreSQL, M, Integration-style tests for schema endpoints (would use TestClient in real scena (+10 more)
...
## Knowledge Gaps
- **2984 isolated node(s):** `PlainTextResponse`, `Path`, `PlainTextResponse`, `Path`, `Any` (+2979 more)
...
- **Why does `TableConfig` connect `TableConfig` to `validate_project.py`, `AutoFixService`, `SEAD Clearinghouse Ingester`, `public.tbl_locations`, `Community 395`, `ProjectDetailView.vue`, `useWhatsNew.ts`, `source`, `TestShapeShiftProject`, `EntityFormDialog.vue`, `generate_schemas.py`, `Community 277`, `_make_operations`, `UnnestConfig`, `SEAD Clearinghouse Ingester - Architecture Overview`, `ExcelDispatcher`, `CreateEntityFromTableDialog.vue`, `QueryBuilder.vue`, `MaterializationService`, `Any`, `SubsetService`, `.is_interpolated_string`, `4. Project Reference`, `03_functions.sql`, `tests/test_utility.py`, `TargetModel`, `CsvDispatcher`, `ReconciliationGrid.vue`, `ForeignKeyConfig`, `01_tables.sql`, `process_merged_branch`, `DataSourceService`, `Community 309`, `models/mapping.py`, `.folder`, `SourceSpan`, `Community 317`, `properties`, `conformance.py`, `Any`, `FilePathResolver`, `DuckDbWorkspace`, `ValidatorRegistry`, `AppendEditor.vue`, `public.tbl_methods`, `EntitySpec`, `EntityMappingManager`, `ForeignKeyConstraints`, `ReconciliationProgressDialog.vue`, `Proposal: BugsCEP Importer Migration Runtime Decision Spike`, `Playwright E2E Testing Setup - Complete! ✅`, `EntityPreviewPanel.vue`, `PreviewResult`, `ReconciliationQueryService`, `SuggestionsPanel.vue`, `.get_subset2`, `ShapeShifter`, `MaterializationSpecification`, `Settings`?**
  _High betweenness centrality (0.120) - this node is a cross-community bridge._
- **Why does `ShapeShiftProject` connect `tests/test_utility.py` to `Project`, `validate_project.py`, `AutoFixService`, `TableConfig`, `ColumnSpec`, `ProjectDetailView.vue`, `TestLifecycleVersionContracts`, `TestShapeShiftProject`, `EntityFormDialog.vue`, `sead/utility.py`, `ValidationError`, `DataFrame`, `get_data_source_status`, `UnnestConfig`, `CreateEntityFromTableDialog.vue`, `SEAD Clearinghouse Ingester - Architecture Overview`, `ExcelDispatcher`, `QueryBuilder.vue`, `Any`, `BranchEditor.vue`, `03_functions.sql`, `TargetModel`, `ForeignKeyConfig`, `DataSourceService`, `models/mapping.py`, `ProjectSession`, `IngesterService`, `.folder`, `TestYamlEntityKeyOrdering`, `SourceSpan`, `properties`, `conformance.py`, `Any`, `EntityOperations`, `FilePathResolver`, `Reconciliation Setup Guide`, `useTaskStatusStore`, `AppendEditor.vue`, `public.tbl_methods`, `TestTaskListSidecarManager`, `EntitySpec`, `EntityMappingManager`, `ReconciliationProgressDialog.vue`, `Playwright E2E Testing Setup - Complete! ✅`, `EntityPreviewPanel.vue`, `ReconciliationQueryService`, `.dispatch`, `ShapeShifter`, `DataSourceConfig`, `MaterializationSpecification`, `useCytoscape`?**
  _High betweenness centrality (0.075) - this node is a cross-community bridge._
- **Why does `enum` connect `.folder` to `MaterializationService`, `validate_project.py`, `AutoFixService`, `FixedLoader`, `Any`, `FixedEntityPersistenceStrategy`, `SeadChangeRequestSimsAdapter`, `OperationManager`, `ReconciliationClient`, `get_data_source_status`, `IngesterService`, `src/utility.py`, `SourceSpan`, `app/models/__init__.py`?**
...
- **Are the 351 inferred relationships involving `ShapeShiftProject` (e.g. with `AuthorizedResource` and `Depends`) actually correct?**
  _`ShapeShiftProject` has 351 INFERRED edges - model-reasoned connections that need verification._
- **Are the 314 inferred relationships involving `Project` (e.g. with `Any` and `AuthorizedResource`) actually correct?**
  _`Project` has 314 INFERRED edges - model-reasoned connections that need verification._
- **Are the 287 inferred relationships involving `ProjectService` (e.g. with `AuthorizedResource` and `Depends`) actually correct?**
  _`ProjectService` has 287 INFERRED edges - model-reasoned connections that need verification._