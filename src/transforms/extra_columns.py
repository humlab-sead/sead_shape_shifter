...
class ExtraColumnEvaluator:
    ...
    def evaluate_interpolation(df: pd.DataFrame, pattern: str, entity_name: str = "") -> pd.Series:
        ...
        if missing:
            entity_suffix = f" for entity '{entity_name}'" if entity_name else ""
            raise ValueError(
                f"Cannot interpolate '{pattern}'{entity_suffix}: "
        ...
        def interpolate_row(row: pd.Series) -> str:
            ...
            for col in columns:
                if pd.isna(row[col]):
                    ...
                    val = row[col]
                    # Convert to string, removing trailing .0 for whole numbers
                    if isinstance(val, (float, int)):
                        # Check if it's a whole number
                        if isinstance(val, float) and val == int(val):
                            values[col] = str(int(val))
                        else:
                            values[col] = str(val)
                    else:
                        values[col] = str(val)
            ...
            temp = pattern.replace('{{', '\x00LEFTBRACE\x00').replace('}}', '\x00RIGHTBRACE\x00')
