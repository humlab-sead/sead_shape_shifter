...
- `datesperiod` dataset and analysis-entity supporting-output fixtures plus `sample` supporting-dimension fixtures now also expose explicit `row_changed` expectations, so those supporting-output contracts record both the supporting action and whether the Java path reports a changed row.
