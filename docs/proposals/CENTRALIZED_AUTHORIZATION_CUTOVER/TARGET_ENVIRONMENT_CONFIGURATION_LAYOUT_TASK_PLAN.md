...
- [ ] Runtime, backup, restore, systemd, nginx, and deployment verification checks pass on the target deployment, or the remaining exceptions are explicitly accepted by the release owner.
...
publication, container configuration, authorization database integrity,
manifest reconciliation, NGINX syntax, and external HTTPS reachability. Same-LAN
testing is not available for this virtual server and is recorded as an
environment limitation. Do not mark this plan fully complete until the release
owner accepts that limitation and the remaining authenticated-access, cleanup,
rollback, and firewall checks, or until those checks record pass results.
