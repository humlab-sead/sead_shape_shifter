...
| `V-8` | Authorization wrapper path check | Run `container/scripts/authorization.sh --help` and controlled backup/restore dry-run commands with `CONFIG_DIR` and `DATA_DIR` overrides | `CFG-AC-3`, `CFG-AC-7` | Wrapper uses external backend env and data state paths | Not run: `authorization.sh` still reads `DATA_DIR/backend.env`, which `T2.2` changes |
