...
- **Status:** Implementation complete; live deployment verification pending
...
| `V-21` | Live target deployment verification | Run the safe post-deployment checks against `/data/test-shape-shifter.sead.se` and record authenticated access, reconciliation, cleanup, and rollback separately | `CFG-AC-3`, `CFG-AC-4`, `CFG-AC-7` | The running deployment is healthy, contained to loopback, uses the external layout, and passes authorization allow/deny, reconciliation, and rollback checks | Partial on 2026-09-22: loopback health returned `200`; the public protected route returned `401`; the backend listened only on `127.0.0.1:8012`; the LAN address refused connections; container configuration passed; and authorization database integrity passed. Positive authenticated access is blocked because the reviewed project files are absent from the deployment. Manifest reconciliation could not be run because the configured manifest file is not available at the host path exposed to this shell. Cleanup, rollback, host firewall listing, and root-only NGINX/systemd installation remain pending |
...
- [ ] Runtime, backup, restore, systemd, nginx, and deployment verification checks pass on the target deployment.
...
The live target checks in `V-21` confirm health, authentication denial, loopback
publication, container configuration, and authorization database integrity. The
remaining release checks need target-operator action: provide or regenerate the
project content named by the reviewed manifest, run the positive authenticated
access check, reconcile the manifest from its actual target-config location,
clean up the four temporary resources, and perform rollback plus the root-only
NGINX/systemd and firewall checks. Do not mark this plan fully complete until
those checks have recorded pass results or an explicitly approved exception.
