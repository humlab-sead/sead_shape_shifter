...
| Credential rotation after the loopback fix | Failed: the `sead_ro` `.pgpass` credential remains pending rotation or an approved exception. Confirmed and recorded as a non-blocking GitHub issue. |
...
- The two latest-run failures are confirmed and recorded as non-blocking GitHub issues: pending credential rotation and incomplete host-log review. The first run also reported transient endpoint-containment and container-log failures; both passed in the three subsequent runs.
