...
| `container/scripts/verify/verify_authenticated_access.sh` | Requires `--base-url`, `--principal-a`, `--principal-b`, `--project-a`, `--project-b`; expects unauthenticated `401`, allowed `200`, and concealed denied `404`; needs each project granted to exactly one principal and neither principal to hold a deployment role that grants read. It now reads those roles from the deployed policy, reports them, and expects `200` for a privileged principal instead of reporting an isolation failure | The reviewed projects are now provisioned, so the check can use reviewed projects instead of temporary ones; no reviewed pair can avoid the role constraint, so the transcript must state the principal scope |
...
* [x] `T4.6` **Change:** Re-run access checks using reviewed projects.
  ...
  * **Validation:** `V-4.7`. Run 2026-09-22 with `bruno` and `riia` against `Bruno-Strucke-v2-test` and `Glykou_etal_2021`: unauthenticated `401`, `bruno` on his own project `200`, `bruno` on `Glykou_etal_2021` concealed `404`, `riia` on her own project `200`, `admin` on the project list `200`. The script's `riia` on `Bruno-Strucke-v2-test` probe reports `200` where it expected `404`; that is a test-selection error, not an access defect, because `riia` holds `project_maintainer`, which grants `READ` on every project in `backend/app/authorization/policy.py`, and `AuthorizationService.is_allowed()` consults deployment roles before resource grants. No reviewed pair can supply a well-formed isolation probe: `roger` and `rebecka` hold `admin`, `riia` holds `project_maintainer`, and `bruno` is the only reviewed owner who is not a global reader. The four required outcomes are recorded in the deployment record, and the raw transcript of both runs (16:37 and 17:01 local) is filed as `<DATA_DIR>/deployment-verification/phase-4/verify-authenticated-access.log`. The script was then corrected to read the deployment roles from the deployed policy, label a privileged read, and report how many isolation directions were verified; the correction passed `bash -n`, `shellcheck -S warning`, and a three-case local harness. The correction was merged to `dev` in PR #500 and reached the target at 19:09, after both filed runs, so the deployment host never exercised it. The post-fix re-run was skipped because the principal passwords are not available from the workstation used, and the residual risk is accepted by the phase owner; see the deployment record section *Post-Fix Re-Run Skipped (Accepted Risk)*. `PH4-AC-2` remains met on the two filed runs.
...
* [x] `T4.8` **Change:** Assemble the evidence pack.
  ...
  * **Validation:** `V-4.10`. The record gained *What Is Not Verified* and *How To Exercise The Deployment* alongside the per-task detail sections, so a tester can act without reading the Phase 3 archive. Limitations are stated as limitations, and acceptance ownership stays outside this record. The `bulgaria-arbodat-lookup-options` limitation was resolved the same day by removing the definition, recorded under *Shared data source decision*.
...
| `PH4-AC-2` | `T4.6` | `V-4.7` | Administrator, owner, denied-principal, and unauthenticated outcomes recorded, with the raw transcript filed |
...
| Access-check evidence | Raw transcript of both pre-fix access-check runs, recovered from the session record | Done | `<DATA_DIR>/deployment-verification/phase-4/verify-authenticated-access.log` |
...
| Area 3: Re-verify access and exercise the procedures | Done | `T4.6` and `T4.7` complete, including the service restart that realigned the unit after the standalone rollback exercise. Both pre-fix access-check runs are filed as `deployment-verification/phase-4/verify-authenticated-access.log`; the corrected script reached the target at 19:09 after those runs. The post-fix re-run was skipped and the residual risk accepted |
...
- **Tester accounts were decided without grant changes.** Acceptance content is exercised through a gatekeeper account (`roger`, `rebecka`, `riia`, `mattias`) or `bruno`, which matches reviewed Decisions 7, 8, and 10 and keeps the manifest checksum, the backup lineage, and the rollback position valid. The four accounts that own no reviewed project cannot reach the reviewed dataset, so they are not used for content acceptance. The record states this and the isolation consequence.
