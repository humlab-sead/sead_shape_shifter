...
- **One data source cannot connect.** `bulgaria-arbodat-lookup-options` is listed by the application but declares no data file. `driver: access` is valid, but the driver schema marks `filename` as `required=True` and the loader falls back to an empty path, so the connection string has no database. No Bulgarian Access file exists in `shared-data/` either. Remove the data source or provision the file, and record the choice as a limitation until then.
