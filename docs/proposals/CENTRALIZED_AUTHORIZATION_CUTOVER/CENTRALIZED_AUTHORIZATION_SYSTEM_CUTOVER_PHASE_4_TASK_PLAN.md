...
* [x] `T4.6` **Change:** Re-run access checks using reviewed projects.
  ...
  * **Validation:** `V-4.7`. Run 2026-09-22 with `bruno` and `riia` against `Bruno-Strucke-v2-test` and `Glykou_etal_2021`: unauthenticated `401`, `bruno` on his own project `200`, `bruno` on `Glykou_etal_2021` concealed `404`, `riia` on her own project `200`, `admin` on the project list `200`. The script's `riia` on `Bruno-Strucke-v2-test` probe reports `200` where it expected `404`; that is a test-selection error, not an access defect, because `riia` holds `project_maintainer`, which grants `READ` on every project in `backend/app/authorization/policy.py`, and `AuthorizationService.is_allowed()` consults deployment roles before resource grants. No reviewed pair can supply a well-formed isolation probe: `roger` and `rebecka` hold `admin`, `riia` holds `project_maintainer`, and `bruno` is the only reviewed owner who is not a global reader. The four required outcomes are recorded in the deployment record. The script was then corrected to read the deployment roles from the deployed policy, label a privileged read, and report how many isolation directions were verified; the correction passed `bash -n`, `shellcheck -S warning`, and a three-case local harness. Re-running the check on the deployment host is the remaining verification, because the in-container role lookup needs it.
