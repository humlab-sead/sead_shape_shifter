...
| Close policy semantics gaps | In progress | First concrete divergence areas are recorded in the datesperiod, datescalendar, sitelocations, and siteotherproxies policies; `species` now exposes explicit supporting_action labels and five simpler reconciliation families now expose explicit persisted_action labels for their write paths. |
| Prepare implementation-oriented fixtures and outputs | In progress | Geochronology graph fixtures carry supporting_action, the site/contact list families carry persisted_action, `species` starts the taxa graph family rollout, and 10 reconciliation-only slices now carry explicit write-action labels. |
...
| Reconciliation write-action baseline | First 10 reconciliation-only slices with explicit persisted_action labels for insert and update write paths | Done | `docs/proposals/BUGSCEP_POLICY_SCHEMA_MACHINE_READABLE_FIDELITY_GAP_INVENTORY.md` |
