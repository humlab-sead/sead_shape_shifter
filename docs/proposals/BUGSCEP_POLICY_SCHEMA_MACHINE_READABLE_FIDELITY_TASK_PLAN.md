...
| Close policy semantics gaps | In progress | First concrete divergence areas are recorded in the datesperiod, datescalendar, sitelocations, and siteotherproxies policies; `species` now exposes explicit supporting_action labels; five simpler reconciliation families expose explicit write-action labels; the first error-and-guard batch covered `period`, `lab`, `bibliography`, `rdbcode`, `rdbsystem`, `ecocodedefinition_bugs`, `ecocodedefinition_koch`, and `speciesassociation`; the next batch extended explicit no-write actions to `site`, `sitereferences`, `rdb`, `taxaseasonality`, `speciesbiology`, `specieskeys`, `speciessynonyms`, and `speciesdistribution`; the following batch extended that same no-write contract to `country`, `mcrnames`, and `taxanotes`; the next batch added trace-hit existing-error coverage for `country`, `mcrnames`, `ecocodegroup`, `ecocode_bugs`, and `ecocode_koch` plus keep-existing and keep-existing-error semantics for `mcrsummary`; the next batch added explicit supporting-action labels for `sample` supporting dimensions and `datasetcontacts` supporting contacts; the next batch extended the same execution-facing contract across all 10 `fossil` scenarios with supporting actions on successful graph branches and explicit `row_changed` expectations on graph issues; the following batch added explicit `row_changed` expectations across all 10 executable `sitelocations` and `siteotherproxies` list-output scenarios; the next batch added explicit `row_changed` expectations across all six executable `species` related-output graph scenarios; the following batch added explicit `row_changed` expectations across the 10 executable `datesperiod` and `sample` supporting-output scenarios; the next batch added explicit `row_changed` expectations across the 10 executable `datesradio` and `datescalendar` supporting-output scenarios; the next batch added explicit `row_changed` expectations across all executable `species` supporting-output scenarios; and the latest batches add explicit `row_changed` expectations across all executable `datasetcontacts` and `fossil` supporting-output or list-output scenarios plus explicit `persisted_action` labels on the successful write paths for `period`, `country`, and `taxanotes`. |
| Prepare implementation-oriented fixtures and outputs | In progress | Geochronology graph fixtures carry supporting_action, `sample` supporting dimensions now expose create, update, keep, and delete supporting actions plus explicit `row_changed` expectations, `datesperiod` supporting outputs now expose explicit `row_changed` expectations for dataset and analysis-entity create, update, and keep paths, `datesradio` and `datescalendar` supporting outputs now also expose explicit `row_changed` expectations for relative-age, dataset, and analysis-entity create, update, keep, and reuse paths, `species` supporting outputs now also expose explicit `row_changed` expectations for family, genus, author, and species create or reuse paths, `datasetcontacts` supporting contacts now expose create and reuse supporting actions plus explicit `row_changed` expectations, the full `fossil` family now exposes supporting actions for dataset and analysis-entity reuse or clone-driven creation plus explicit graph-issue change-state expectations, and the fossil supporting-output scenarios now also expose explicit `row_changed` expectations for dataset and analysis-entity creation or reuse; `datasetcontacts`, `sitelocations`, and `siteotherproxies` now expose explicit `row_changed` expectations across all executable list-output scenarios; `species` now exposes explicit `row_changed` expectations across all executable related-output graph scenarios; and the no-write contract now covers more of the ordered-reconciliation corpus. |
...
| Validate and sync companion docs | In progress | Focused validation is green for the latest slices, `make validate-policy-format` is green after the datasetcontacts, fossil, period, country, and taxanotes batches, and the remaining work is to keep the companion proposal set aligned as more families are upgraded. |
...
- [x] `docs/proposals/BUGSCEP_POLICY_SCHEMA_MACHINE_READABLE_FIDELITY.md` and this task plan both describe the same end-game and remaining work.
...
| Site and contact persisted-action contracts | Execution-facing contract for append, keep, delete, replace, prerequisite-stop behavior, and explicit list-output change-state expectations | Done | `docs/proposals/BUGSCEP_POLICY_PERSISTED_ACTION_CONTRACTS_SITE_CONTACTS.md` |
