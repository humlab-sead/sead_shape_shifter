...
- Implemented so far: direct related-output references, structured resolvers, grouped postprocess merge stages, shared `emit` blocks, initial `known_divergences` support, fixture-backed scenario validation, shared result-object comparisons against current Java behavior, execution-facing `supporting_action` and `persisted_action` labels in the first upgraded reference families, and narrow policy-side execution for resolver, postprocess, ordered-reconciliation, supporting-output, and related-output graph slices
