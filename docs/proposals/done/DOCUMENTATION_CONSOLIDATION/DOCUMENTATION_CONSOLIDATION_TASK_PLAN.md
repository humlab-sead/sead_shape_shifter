...
- `container/` is tracked in git (31 files), so Area 2 can delete `docker/` without losing the Podman deployment path.

**Acceptance Criteria**

- [x] `PH1-AC-1` Root `README.md` contains no developer setup walkthrough, no deployment procedure, no CLI or YAML reference, and no architecture deep-dive; it stays under 300 lines and links to the owning documents.
- [x] `PH1-AC-2` `docs/OPERATIONS.md` documents the Podman deployment as the only deployment path, names no `docker/` path or `docker compose` command, and stays within the 800–1800 word target in `.github/instructions/operations.instructions.md`.
- [x] `PH1-AC-3` Every relative link in `README.md`, `docs/README.md`, `docs/OPERATIONS.md`, `docs/USER_GUIDE.md`, and `docs/DIAGRAMS.md` resolves; finding 3 is recorded as not reproducible with the evidence that disproves it.
- [x] `PH1-AC-4` `docs/USER_GUIDE.md` contains no YAML semantics owned by `docs/CONFIGURATION_GUIDE.md` and no broken links.
- [x] `PH1-AC-5` `docs/DIAGRAMS.md` contains diagrams with short captions only; product narrative and the feature catalog live in `docs/DESIGN.md` or `docs/USER_GUIDE.md`.
- [x] `PH1-AC-6` `docs/README.md` is a documentation map with one row per active document, lists every active document, and contains no duplicated proposal inventory.
- [x] `PH1-AC-7` A `make` target runs the documentation link check, and the retired `docker/` path no longer appears in the root `Makefile`, `README.md`, `docs/OPERATIONS.md`, or `.github/instructions/`.

---

## Repository Findings

**Repository basis:** branch `documentation-consolidation`, commit `5b12060d`, reviewed 2026-09-16. The recent container deployment, operations documentation, and proposal archive changes are included in this baseline. The working tree was clean when this review was performed. The review findings lived in a scratch file, `TEMP.md`, which was removed on 2026-09-10.
...
| `docs/OPERATIONS.md` (590 lines, 3949 words) | 60 `docker` mentions, 0 `podman` mentions. The recent Basic-auth user-management section is present, but paths, commands, and image names still describe the Docker path. | Area 3 rewrites it. Instruction target is 800–1800 words. |
| `container/README.md`, `container/DEPLOYMENT.md`, `container/Makefile`, `container/scripts/setup.sh`, `container/podman-compose.yml` | Podman deployment with data at `../container-data/`. Makefile targets include `setup`, `build`, `build-local`, `up`, `down`, `restart`, `logs`, `status`, `ps`, `shell`, `healthcheck`, `backup`, `clean`, `prune`, and `service-*`. Container name is `shape-shifter`. | These are the authoritative sources for Area 3 and Area 4 content. |
...
**Affected files:** `docs/README.md`, `docs/USER_GUIDE.md`, `docs/proposals/done/`, `docs/proposals/SHAPESHIFTER_PROJECT_AI_ADVISOR/`

**Dependencies:** None. Independent of Areas 2–6, except `T1.6`, which runs last.

**Tasks**

- [x] `T1.1` **Complete:** Correct the broken link at `docs/USER_GUIDE.md:533`.
...
- [x] `T1.2` **Complete:** Correct the three broken proposal links in `docs/README.md`.
  - **Target:** `docs/README.md`, "Future proposals" list.
  - **Current → required:** links to `proposals/future/AI_PROJECT_ADVISOR_PROPOSAL.md`, `proposals/future/COMMENT_PRESERVING_SAVE_PATH.md`, and `proposals/future/COMMENT_PRESERVING_SAVE_PATH_IMPLEMENTATION_SKETCH.md` → `proposals/SHAPESHIFTER_PROJECT_AI_ADVISOR/SHAPESHIFTER_PROJECT_AI_ADVISOR.md`, `proposals/done/COMMENT_PRESERVING_SAVE_PATH.md`, and `proposals/done/COMMENT_PRESERVING_SAVE_PATH_IMPLEMENTATION_SKETCH.md`.
  - **Implementation:** Repoint the AI advisor entry to its current proposal and the two comment-preserving entries to their archived files. `T6.1` replaces the whole inventory, so keep this edit minimal.
  - **Validation:** `V-1` reports zero broken links in `docs/README.md`.

- [x] `T1.3` **Complete:** Record finding 3 as not reproducible.
  - **Target:** this task plan and `Acceptance-Criteria Coverage`.
  - **Current → required:** finding 3 is unreviewed → finding 3 is closed with evidence.
  - **Implementation:** Already recorded in `Repository Findings`. Do not create a `DOCKER_TO_PODMAN_MIGRATION.md` link fix task.
  - **Validation:** `V-8`.

- [x] `T1.4` **Complete:** Retain `docs/REQUIREMENTS.md` as the requirements reference.
  - **Target:** `docs/REQUIREMENTS.md`, `docs/README.md`, `docs/archive/`.
  - **Current → required:** listed in `docs/README.md` with a scope claim that overlaps `DESIGN.md` and `USER_GUIDE.md` → retained as the distinct functional and non-functional requirements baseline and listed with that scope.
  - **Implementation:** `REQUIREMENTS.md` defines FR/NFR identifiers, personas, use cases, constraints, and success criteria that are not owned by the design or user guide. It remains active and is listed in the Architecture And Requirements section of `docs/README.md`.
  - **Validation:** `V-1`, `V-8`.

- [x] `T1.5` **Complete:** Rewrite `docs/README.md` as a documentation map.
  ...
  - **Implementation:** Replaced the duplicated proposal inventory, repeated navigation, version history, and last-updated footer with an 83-line map. The map links to active guides, requirements, security references, target-model references, Podman runbooks, proposal status directories, and current proposal entry points.
  - **Constraints:** Keep `docs/README.md` in the same location with the same title. Do not move other documents.
  - **Validation:** `V-1`, `V-2`, `V-8`; `docs/README.md` is 83 lines and all listed targets resolve.

- [x] `T1.6` **Complete:** Add documentation link check as a repeatable command.
  ...
  - **Implementation:** Add `NEW scripts/check_doc_links.sh` containing the verified scan used for `V-1`, covering active root, `docs/`, and `container/` Markdown while excluding nested historical and generated documentation. Add a `check-doc-links` target to the root `Makefile` and append it to the `lint` dependency list. Add one line to `docs/DEVELOPMENT.md` under its code-quality section. Do not add a new GitHub workflow; `release.yml` is release-only.
  - **Constraints:** Shell only, no new Python or Node dependency. Must be runnable from the repository root.
  - **Validation:** `V-1`, `V-7`; `make check-doc-links` passes for active root, docs, and container Markdown.
...
**Affected files:** `docker/**`, `Makefile`, `.github/instructions/`, `docs/proposals/done/MITIGATE_SECURITY_ISSUES/SECURITY_CHECK.md`

**Dependencies:** `container/` is already tracked. Area 3 rewrites `docs/OPERATIONS.md`, which is the last inbound reference.

**Tasks**

- [x] `T2.1` **Complete:** `container/` is tracked in version control.
  - **Target:** `container/` (tracked in the current tree).
  ...
  - **Implementation:** Already satisfied by the container deployment history beginning at `db71c5df`; 31 files are tracked. No additional commit is required before proceeding with the remaining Area 2 tasks.
  - **Validation:** `V-3`; `git ls-files container/` returns the deployment files.
  - **Dependencies:** Satisfied; `T2.2` and `T2.3` may proceed once their other documentation prerequisites are complete.

- [x] `T2.2` **Complete:** Remove root `Makefile` Docker hooks.
  ...
  - **Constraints:** The root `Makefile` must still parse. `make -n frontend-coverage` is the available parse check because the root Makefile has no `help` target.
  - **Validation:** `V-2`, `V-5`; `make -n frontend-coverage` parses successfully. The plan's earlier `make help` check is invalid because the root Makefile has no `help` target.

- [x] `T2.3` **Complete:** Remove the `docker/` directory.
  ...
  - **Constraints:** Do not delete `frontend/Dockerfile`, the root `.dockerignore`, `container/.dockerignore`, `container/Containerfile`, or `container/`.
  - **Validation:** `V-3`, `V-4`; the tracked `docker/` directory is absent and the `container/` deployment remains present.

- [x] `T2.4` **Complete:** Update agent instruction files that named `docker/` as a trusted source.
  - **Target:** `.github/instructions/operations.instructions.md:72,73,79`; `.github/instructions/readme.instructions.md:21,30,46`.
  - **Current → required:** trusted sources `docker/docker-compose.yml`, `docker/Dockerfile`, `docker/Makefile`, `docker/build.sh` and the README link list entry `docker/README.md` → `container/podman-compose.yml`, `container/Containerfile`, `container/Makefile`, `container/scripts/build.sh`, `container/README.md`.
  ...
  - **Validation:** `V-3`, `V-7`; all replacement paths exist and the active instruction files contain no `docker/` references.

- [x] `T2.5` **Complete:** Annotate historical `docker/` references in the security proposal.
  ...
  - **Validation:** `V-8`; the note was added without changing the report findings.
...
- [x] `T3.1` **Complete:** Confirm the current deployment topology.
  ...
  - **Validation:** `V-4`; the Podman topology and named commands were checked against `container/README.md`, `container/DEPLOYMENT.md`, and `container/Makefile`.

- [x] `T3.2` **Complete:** Update paths, commands, and image handling.
  - **Target:** `docs/OPERATIONS.md`, especially the runtime configuration, data layout, build artifacts, deployment flow, rollback, and verification sections.
  ...
  - **Implementation:** Replace each path and command with the verified `container/` equivalent. `container/scripts/setup.sh` creates `../container-data/{projects,shared,logs,output,backups,tmp,state,.pgpass}` and `backend.env`; use that layout in the volume table. The container name remains `shape-shifter`, so `backend/app/scripts/authorization.py` commands keep the same module path and change only the `docker compose exec` prefix.
  - **Constraints:** Both `ops` and the container Makefile must agree; if they disagree, the Makefile and `setup.sh` win.
  - **Validation:** `V-3`, `V-4`; the guide has no Docker references and all named Make targets resolve.

- [x] `T3.3` **Complete:** Rebuild the runtime environment variable table against `backend/app/core/config.py`.
  ...
  - **Validation:** `V-4`; the documented variables and defaults were checked against `config.py`.

- [x] `T3.4` **Complete:** Update deployment flow, rollback, and references.
  - **Target:** `docs/OPERATIONS.md`, especially the deployment flow, rollback, and references sections.
  ...
  - **Validation:** `V-3`, `V-4`; references point to the Podman runbooks and the release and rollback commands resolve.

- [x] `T3.5` **Complete:** Trim to the 800–1800 word target.
  - **Target:** `docs/OPERATIONS.md` (3949 words).
  ...
  - **Validation:** `V-2`; the resulting guide is 1118 words.
...
- [x] `T4.1` **Complete:** Replace the Docker quick start with a Podman quick start.
  ...
  - **Validation:** `V-3`, `V-4`; the README uses the supported `container/` targets and links to the Podman runbooks.

- [x] `T4.2` **Complete:** Remove developer installation and command sections.
  ...
  - **Validation:** `V-1`, `V-2`; the README is 60 lines and points to `docs/DEVELOPMENT.md`.

- [x] `T4.3` **Complete:** Remove CLI usage, YAML example, database integration, and FK constraint sections.
  ...
  - **Validation:** `V-1`; configuration and user-guide links provide the owning documentation.

- [x] `T4.4` **Complete:** Remove architecture, testing, and contributing sections.
  ...
  - **Validation:** `V-1`, `V-2`; the README retains only a short capability summary, API note, and documentation links.
...
- [x] `T5.1` **Complete:** Move narrative sections out of `docs/DIAGRAMS.md`.
  ...
  - **Validation:** `V-2`, `V-6`; the document now contains concise captions and current diagrams only.

- [x] `T5.2` **Complete:** Fix the section sequence.
  ...
  - **Validation:** `V-6`; heading extraction shows a continuous 1..7 sequence.

- [x] `T5.3` **Complete:** Trim captions and length.
  ...
  - **Validation:** `V-2`, `V-6`; seven current diagrams remain in 155 lines with one-sentence captions.

- [x] `T5.4` **Complete:** Audit `docs/USER_GUIDE.md` for YAML semantics owned by the configuration guide.
  ...
  - **Validation:** `V-1`, `V-6`; the guide links to the configuration reference for schema and identity semantics.
...
- [x] `T6.1` **Complete:** Review the complete diff for unintended changes.
  ...
  - **Validation:** `V-8`; the final diff contains no changes under `src/`, `backend/app/`, `frontend/src/`, `ingesters/`, or `tests/`.

- [x] `T6.2` **Complete:** Verify the Makefile still parses after `T2.2`.
  - **Target:** root `Makefile`.
  - **Current → required:** an include of a deleted file → a self-contained `Makefile`.
  - **Implementation:** Run `make -n frontend-coverage` and confirm exit code 0.
  - **Validation:** `V-5`; `make -n frontend-coverage` exits 0.

**Completion evidence:** `git diff --name-only` matches Scope, and `make -n frontend-coverage` exits 0.
...
| `V-2` | Document size against instruction targets | `wc -l README.md docs/README.md docs/DIAGRAMS.md && wc -w docs/OPERATIONS.md` | `PH1-AC-1`, `PH1-AC-2`, `PH1-AC-5`, `PH1-AC-6` | `README.md` < 300 lines; `docs/README.md` < 150 lines; `docs/DIAGRAMS.md` reduced; `docs/OPERATIONS.md` 800–1800 words | Fail: 502 / 302 / 1397 lines; 3949 words |
| `V-3` | No active retired `docker/` deployment references | `git grep -n -iE 'docker/' -- ':!graphify-out' ':!CHANGELOG.md' ':!docs/archive' ':!docs/proposals/done/MITIGATE_SECURITY_ISSUES/SECURITY_CHECK.md' ':!docs/proposals/DOCUMENTATION_CONSOLIDATION/DOCUMENTATION_CONSOLIDATION_TASK_PLAN.md' ':!.vscode/settings.json'` | `PH1-AC-2`, `PH1-AC-7` | No active deployment references | Pass: only the task tracker, editor exclusion settings, and historical records retain the term |
...
| `V-5` | Root `Makefile` parses after include removal | `make -n frontend-coverage` | `PH1-AC-7` | Exit code 0 | Pass: `make -n frontend-coverage` succeeds |
...
|---|---|---|---|
| Repaired links | Four verified broken relative links corrected | Done | `README.md`, `docs/README.md`, `docs/USER_GUIDE.md` |
| Documentation map | `docs/README.md` rebuilt with one row per active document | Done | `docs/README.md` |
| Link-check script | New shell script plus `make check-doc-links`, wired into `lint` | Done | `scripts/check_doc_links.sh`, `Makefile` |
| Podman operations guide | `docs/OPERATIONS.md` rewritten for `container/` | Done | `docs/OPERATIONS.md` |
| Trimmed front door | `README.md` reduced to overview, prerequisites, quick start, and links | Done | `README.md` |
| Narrowed diagrams | Narrative moved out, numbering fixed, captions only | Done | `docs/DIAGRAMS.md` |
| De-duplicated user guide | YAML semantics replaced by links to the configuration guide | Done | `docs/USER_GUIDE.md` |
| Retired Docker path | `docker/` deleted with `Makefile` and instruction files updated | Done | `docker/`, `Makefile`, `.github/instructions/` |
...
| Area | Status | Notes |
|---|---|---|
| 1. Documentation navigation | Done | `T1.1`–`T1.6` complete; active-document link checking passes. |
| 2. `docker/` retirement | Done | `T2.1`–`T2.5` complete; the retired directory is absent and active deployment references are removed. |
| 3. `docs/OPERATIONS.md` rewrite | Done | `T3.1`–`T3.5` complete; Podman-only operations overview validated at 1118 words. |
| 4. Root `README.md` trim | Done | `T4.1`–`T4.4` complete; front door validated at 60 lines. |
| 5. Duplication reduction | Done | `T5.1`–`T5.4` complete; diagrams and user-guide ownership checks pass. |
| 6. Scope confirmation | Done | `T6.1`–`T6.2` complete; no application or test paths changed. |

---

## Definition Of Done

- [x] Every `PH1-AC-*` criterion is satisfied and its evidence is recorded.
- [x] `V-1` passes with zero broken links, and `scripts/check_doc_links.sh` is wired into `make lint`.
- [x] `V-2` passes against the size targets for `README.md`, `docs/README.md`, `docs/DIAGRAMS.md`, and `docs/OPERATIONS.md`.
- [x] `V-3` finds no active `docker/` deployment references; the task tracker, editor settings, and historical records are explicitly retained.
- [x] `V-4` confirms every command and path named in `README.md` and `docs/OPERATIONS.md` exists under `container/`.
- [x] `V-5` confirms `make -n frontend-coverage` exits 0 after the `docker/Makefile` include is removed; the root Makefile has no `help` target.
- [x] `V-6` confirms diagram numbering is continuous and no document explains content owned by another document.
- [x] `V-7` confirms both edited instruction files name only existing paths.
- [x] `V-8` confirms the diff contains no application, test, or runtime configuration changes.
- [x] Finding 3 is closed as not reproducible with the evidence recorded.
- [x] New follow-up work created by this phase is recorded. The two items previously listed here are resolved in the current tree (`podman-compose.yml` `env_file` path, and `resources/backend.env.example` variable names), so no follow-up is needed for them.
