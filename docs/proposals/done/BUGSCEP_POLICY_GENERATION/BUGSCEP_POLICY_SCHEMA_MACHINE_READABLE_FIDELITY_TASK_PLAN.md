...
- [x] The plan defines a common policy-readiness target that supports both end-game options up to the point where they intentionally diverge.
- [x] Remaining Java-only behavior is classified as either policy detail to add, explicit non-goal, or implementation-specific adapter logic.
- [x] At least one representative importer family has policy coverage detailed enough to describe end-to-end execution, including reconciliation behavior, postprocess rules, output side effects, and validation expectations.
- [x] The policy artifacts and fixture corpus are organized so they can be used as implementation inputs, not only as test fixtures.
- [x] Focused validation passes before `make validate-policy-format` runs.
- [x] The fidelity proposal and this task plan stay aligned with the new end-game.
...
- [x] Prioritize gaps that block both end-game options first, especially behaviors that affect row identity, matching, range merging, dataset or analysis-entity creation, and update versus insert rules.
...
- [x] Add explicit `persisted_action` labels to all remaining Tier D policy reconciliation fixtures (11 policies converted 2026-06-20).
- [x] Convert Tier C policies to Tier A by adding structured resolvers and `known_divergences` sections (11 policies converted 2026-06-20).

**Completion Criteria**

The highest-value runtime behaviors that still block implementation are either encoded in policy or explicitly classified as adapter-only logic. **Complete** — all 35 policies are Tier A.
...
- [x] Keep the result shapes aligned with the current shared contracts such as `resolver_result`, `reconciliation_result`, `postprocess_result`, `postprocess_results`, `graph_result`, `graph_issue`, and `output_result` unless a stronger implementation-facing shape is clearly needed.

**Completion Criteria**

There is a representative policy-plus-fixture subset that can act as a golden execution reference for either implementation path. **Complete** — all 35 policies have fixture-backed execution references.
...
- [x] Update `docs/proposals/BUGSCEP_POLICY_GENERATION/BUGSCEP_POLICY_SCHEMA_MACHINE_READABLE_FIDELITY.md` if the coverage statement, non-goals, or recommendation changes under the new end-game.
- [x] Keep this task plan aligned with the current remaining work instead of repeating completed slice history.

**Completion Criteria**

Focused validation passes first, the broad validation target stays green, and the proposal plus task plan reflect the same implementation-oriented end-game. **Complete** — 355 tests pass, all docs synced.
...
|---|-----|----------------|----------------------|--------|
| 1 | End-to-end identity and supporting-output flow | Both implementation paths need a complete contract for how parent rows, supporting rows, and related outputs share identity and reuse state | All 35 policies | **Done** — all 35 policies are Tier A with full coverage |
| 2 | Persisted side effects and output result semantics | Implementation needs to know not only which branch fired, but what row actions and side effects must happen | All 35 policies | **Done** — all policies have explicit action labels and `row_changed` expectations |
| 3 | Helper-derived decision rules hidden in Java behavior | Policies cannot serve as a build contract while important lookup or derivation behavior still depends on Java-only helpers | All 35 policies | **Done** — 34 policies have structured resolvers; 1 policy (`birmbeetledata`) has adapter-only helper documented |
| 4 | Postprocess and graph behavior as full execution contracts | Downstream implementation needs clearer expectations around retained rows, emitted rows, graph issues, and ordering-sensitive outputs | `datescalendar`, `datesperiod`, `datesradio`, `fossil`, `species` | **Done** — all Tier A policies have full coverage |
| 5 | Known divergences and intentional adapter boundaries | Both paths need explicit documentation of what the policy owns, what the adapter owns, and where Java behavior is preserved only for parity | All 35 policies | **Done** — 33 policies have `known_divergences` sections; 2 policies have no divergences to record |
...
| Taxa Graph | `species`, `speciesassociation`, `speciesbiology`, `specieskeys`, `speciessynonyms` | Related-output graphs, optional supporting outputs, repository reuse, multi-node output structure, known divergences, structured resolvers | A (all execution-ready) |
...
The schema design and policy conversion work is complete. The following slices are forward-looking extensions:

1. ~~**Promote Tier D to Tier C**~~ — **Done** (2026-06-20): all 11 Tier D policies promoted to Tier A
2. ~~**Convert Tier D helpers to structured resolvers**~~ — **Done** (2026-06-20): all helper calls converted or documented as adapter-only
3. ~~**Add known_divergences to Tier D**~~ — **Done** (2026-06-20): all policies have `known_divergences` or explicit statement of none
...
5. **Start downstream implementation planning** — choose between Python runtime or Shape Shifter + BugCEP reconciliation path
...
| Close policy semantics gaps | **Done** | All 35 policies are Tier A with execution-facing action labels and `row_changed` expectations. Detailed batch history: see [WRAP_UP_BUGCEP_POLICY.md](WRAP_UP_BUGCEP_POLICY.md#fidelity-work-changelog). |
| Prepare implementation-oriented fixtures and outputs | **Done** | All 35 policies have fixture-backed execution references with explicit action labels. Detailed batch history: see [WRAP_UP_BUGCEP_POLICY.md](WRAP_UP_BUGCEP_POLICY.md#fidelity-work-changelog). |
...
| Validate and sync companion docs | **Done** | 355 tests pass, all docs synced. Proposal and task plan reflect completion. |

## Definition Of Done

- [x] A shared execution-readiness checklist exists for BugsCEP policies.
- [x] The next phase improves policy detail toward implementation readiness, not only broader test-harness execution.
- [x] Representative policies and fixtures can act as a concrete build contract for either downstream option up to the documented divergence point.
- [x] Policy-managed behavior and adapter-only behavior are explicitly separated.
...
| Golden reference fixture set | Representative policy-plus-fixture cases suitable for implementation and regression work | **Done** | `docs/proposals/BUGSCEP_POLICY_GENERATION/BUGSCEP_POLICY_GOLDEN_REFERENCE_GEOCHRONOLOGY.md` |
