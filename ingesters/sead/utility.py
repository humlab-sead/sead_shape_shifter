...
def env2dict(prefix: str, data: dict[str, str] | None = None, lower_key: bool = True) -> dict[str, str]:
    ...
    prefix_to_match = prefix.lower() if lower_key else prefix

    # Expected format: PREFIX_KEY_NAME (prefix + underscore + key)
    expected_prefix = prefix_to_match + "_"
    ...
    for key, value in os.environ.items():
        ...
        elif key.startswith(prefix_to_match) and len(key) > len(prefix_to_match):
            # Key starts with prefix but missing underscore separator - warn and skip
            logger.warning(
                f"Skipping env var '{key}' - starts with prefix '{prefix_to_match}' "
