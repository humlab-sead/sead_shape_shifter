    ...
    .map(c => {
      ...
      switch (c.operator) {
        ...
        case 'NOT IN':
          ...
          const values = c.value.split(',').map(v => escapeLiteral(v.trim())).join(', ');
...
const buildOrderByClause = (): string => {
  ...
  const clauses = orderBy.value
    .filter(o => o.column)
    .map(o => `${escapeIdentifier(o.column)} ${o.direction}`);
...
const generateSql = () => {
  ...
  // Build SELECT clause
  const columns = selectedColumns.value.length === availableColumns.value.length
    ? '*'
...
watch([selectedColumns, conditions, conditionLogic, orderBy, limit], () => {
