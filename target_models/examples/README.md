...
- `sead_canonical_minimal.yml` — hand-authored non-Arbodat positive-control fixture using canonical SEAD-facing names
...
- `sead_canonical_minimal.yml` is hand-authored as a small positive-control project shape.
- `sead_arbodat_core.yml` and `sead_missing_sample_group.yml` are derived from the Arbodat project in `tests/test_data/projects/arbodat/shapeshifter.yml`.
- All fixtures keep only the iteration-1 SEAD core entities relevant to `target_models/specs/sead_v2.yml`.
